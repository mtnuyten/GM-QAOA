...
def maxcut_hamiltonian(n_wires: int, edges: List[Tuple[int, int]], normalize: bool = True) -> qml.Hamiltonian:
    """
    H_P = sum_{(i,j) in E} 0.5*(1 - Z_i Z_j).
    If normalize=True, divide by |E|.
    """
    m = len(edges)
    coeffs = []
    ops = []
...
def empirical_variance(
    ...
    cost_h = maxcut_hamiltonian(n_wires, edges, normalize=True)
...
def plot_variance_data(results, n_wires: int, max_depth: int, outdir: str = " .. /figures", show: bool = True):
    """
    ...
    for g in range(len(next(iter(results.values())))):
        ...
        ax.plot(depths, variances, marker="none", label=f"Graph {g}")
    ...
    save_plot(fig, outdir, f"variance_vs_depth_n_{n_wires}.png", show=show)
    save_plot(fig, outdir, f"variance_vs_depth_n_{n_wires}.pdf", show=show)


def plot_variance_mean_std(results, n_wires: int, max_depth: int, outdir: str = ".. /figures", show: bool = True):
    """
    ...
    fig, ax = plt.subplots(figsize=(3.5, 3.0))
    ax.plot(depths, means, marker="none", color="b", label="Mean variance")
    ...
    save_plot(fig, outdir, f"variance_mean_n_{n_wires}.png", show=show)
    save_plot(fig, outdir, f"variance_mean_n_{n_wires}.pdf", show=show)